-- name: UserCreate :one
-- Creates a user and returns the user record
INSERT INTO tenant.users (name, external_ref)
VALUES (@name::text, @external_ref::text)
RETURNING id, name, external_ref, created;
...
-- Soft-delete the organization membership for a user (by organization and user name)

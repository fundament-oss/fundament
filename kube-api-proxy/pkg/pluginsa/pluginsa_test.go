...
func (f *fakeClient) ResolvePluginSA(_ context.Context, clusterID, installationName string) (*gardener.PluginSAToken, error) {
	f.calls++
	f.lastClusterID = clusterID
	f.lastInstallation = installationName
	if f.err != nil {
		return nil, f.err
	}
	return f.token, nil
...
func TestStub_ReturnsFixedToken(t *testing.T) {
	tok, err := Stub{}.Resolve(context.Background(), "any-cluster", "any-install")
...
func TestReal_ReturnsTokenAndHash(t *testing.T) {
	...
	tok, err := r.Resolve(context.Background(), "cluster-abc", "cert-manager")
	require.NoError(t, err)
	...
	assert.Equal(t, "cert-manager", fake.lastInstallation)
}
...
func TestReal_ClientErrorPropagates(t *testing.T) {
	...
	_, err := r.Resolve(context.Background(), "cluster-abc", "cert-manager")
	require.Error(t, err)

...
// userSANamespace is the namespace cluster-worker provisions per-user SAs in.
// See cluster-worker/pkg/client/shoot/access.go:12 for the constant and
// cluster-worker/pkg/handler/usersync/mod.go for the SA-name construction.
const userSANamespace = "fundament-system"

// userSAPrefix is the SAR Spec.User format: system:serviceaccount:{ns}:fundament-{userID}.
const userSAPrefix = "system:serviceaccount:" + userSANamespace + ":fundament-"

// userSAGroups are the virtual groups the API server attaches to any token for
// an SA in userSANamespace. They must be passed to the SAR so a grant bound to
// one of these groups (not the individual SA) is honoured — see
// gardener.Client.SubjectAccessReview.
var userSAGroups = []string{
	"system:serviceaccounts",
	"system:serviceaccounts:" + userSANamespace,
...
type Client interface {
	SubjectAccessReview(ctx context.Context, clusterID string, user string, groups []string, attrs authorizationv1.ResourceAttributes) (bool, error)
}
...
func (r *UserAccess) Check(ctx context.Context, clusterID string, attrs *kubereq.Attributes, userID string) (bool, error) {
	...
	allowed, err := r.client.SubjectAccessReview(ctx, clusterID, userSAPrefix+userID, userSAGroups, sarAttrs)
	if err != nil {

...
func (f *fakeClient) SubjectAccessReview(_ context.Context, clusterID, user string, groups []string, attrs authorizationv1.ResourceAttributes) (bool, error) {
	f.calls++
...
func TestReal_ForwardsAttrsAndReturnsAllowed(t *testing.T) {
	...
	assert.Equal(t, []string{
		...
		"system:serviceaccounts:fundament-system",
		"system:authenticated",
	}, fake.lastGroups,
		"SAR must carry the SA's virtual groups so group-bound grants are visible")

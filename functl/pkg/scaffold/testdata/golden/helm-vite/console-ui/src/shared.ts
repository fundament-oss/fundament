// Shared helpers for the demo console views.

import type { FundamentSdk } from './sdk.ts';
import type { Condition, Widget } from './types.ts';
...
// Loads the shared NLDD Design System bundle from the host; every <nldd-*>
// element is registered once nldd-design-system.js has run. Views opt in by
// calling this alongside loadSdk().
...
export function phase(item: Widget): string {
...
// under FUN-17; falls back to '*' before init.
function postToHost(message: unknown): void {
  window.parent.postMessage(message, window.fundament?.parentOrigin ?? '*');
}

// The host resolves the destination relative to the iframe's current route, so
// the view only sends the resource identity.
...
export function renderConditionsTable(item: Widget): string {

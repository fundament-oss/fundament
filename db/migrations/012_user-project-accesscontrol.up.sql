...
CREATE OR REPLACE FUNCTION tenant.is_cluster_in_organization(p_cluster_id uuid)
 RETURNS boolean
 LANGUAGE sql
...
ALTER FUNCTION tenant.is_cluster_in_organization(p_cluster_id uuid) OWNER TO fun_authz;

CREATE OR REPLACE FUNCTION tenant.is_project_in_organization(p_project_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE PARALLEL SAFE SECURITY DEFINER COST 1
AS $function$
...
ALTER FUNCTION tenant.is_project_in_organization(p_project_id uuid) OWNER TO fun_authz;

CREATE OR REPLACE FUNCTION tenant.is_user_in_organization(p_user_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE PARALLEL SAFE SECURITY DEFINER COST 1
AS $function$
...
ALTER FUNCTION tenant.is_user_in_organization(p_user_id uuid) OWNER TO fun_authz;
...
CREATE OR REPLACE FUNCTION tenant.is_project_member(p_project_id uuid, p_user_id uuid, p_role text)
 RETURNS boolean
 LANGUAGE sql
 STABLE PARALLEL SAFE SECURITY DEFINER COST 1
AS $function$
...
ALTER FUNCTION tenant.is_project_member(p_project_id uuid, p_user_id uuid, p_role text) OWNER TO fun_authz;
...
CREATE POLICY "project_members_select_policy" ON "tenant"."project_members"
    ...
    USING ((tenant.is_project_in_organization(project_id) AND (tenant.is_project_member(project_id, tenant.current_user_id(), NULL::text) OR (user_id = tenant.current_user_id()))));

CREATE POLICY "project_members_insert_policy" ON "tenant"."project_members"
    AS PERMISSIVE
    FOR INSERT
    TO fun_fundament_api
    WITH CHECK ((tenant.is_project_in_organization(project_id) AND ((deleted IS NOT NULL) OR tenant.is_user_in_organization(user_id)) AND (tenant.is_project_member(project_id, tenant.current_user_id(), 'admin'::text) OR (NOT tenant.project_has_members(project_id)))));

CREATE POLICY "project_members_update_policy" ON "tenant"."project_members"
    AS PERMISSIVE
    FOR UPDATE
    TO fun_fundament_api
    USING ((tenant.is_project_in_organization(project_id) AND tenant.is_project_member(project_id, tenant.current_user_id(), 'admin'::text)));
...
CREATE POLICY "projects_delete_policy" ON "tenant"."projects"
	...
	FOR DELETE
	TO fun_fundament_api
	USING (((organization_id = tenant.current_organization_id()) AND tenant.is_project_member(id, tenant.current_user_id(), 'admin'::text)));

...
At 50,000 servers across 3+ sites, the physical network architecture must be explicitly chosen. The topology determines scalability, fault domains, and whether network provisioning can be fully automated alongside compute provisioning.
...
* Cons: spanning tree limits scale, poor east-west performance, difficult to automate, does not scale to 50k servers
...
Spine-leaf with BGP/EVPN. Only topology proven to scale to 50,000+ servers with full automation. Aligns with the automation-first requirement from ADR-0002.

...
* The org-api → ingress TLS needs a CA knob (`prometheus.WithTransport`), wired as `PROMETHEUS_CA_FILE` / `organizationApi.prometheusCASecret`.

[NOTE]
====
Amended 2026-09-02. This ADR originally recorded that "real landscapes with publicly-trusted ingress certs need nothing", i.e. that one static bundle covers every shoot. That holds only where the seed carries a wildcard certificate (a Secret labelled `gardener.cloud/role=controlplane-cert` in the seed's `garden` namespace). Otherwise gardenlet signs each shoot's Prometheus, Plutono, Vali and Alertmanager ingress certificate with *that shoot's own cluster CA* (`SigningCA: SecretNameCACluster`, gardener/gardener v1.139.4 `pkg/gardenlet/operation/botanist/monitoring.go`), so a static bundle silently loses every cluster created after it was built.

Org-api therefore resolves the CA per shoot, from the `<shoot>.ca-cluster` ConfigMap in the same project namespace as `<shoot>.monitoring`. A Gardener credential that cannot read the ConfigMap fails resolution, since the ingress does not verify without the CA. Trust is additive: whatever roots the image has, the operator-supplied bundle, and the shoot's own CA are all anchors. `prometheusCASecret` is now an extra anchor rather than the only one — and it is still the way to trust a seed-wide wildcard certificate, public or private, because org-api ships in a `scratch` image with no root store.

The shoot CA, like the monitoring URL and credentials, is trusted because only Fundament's own Gardener credential is a member of the Gardener projects: project members can write `<shoot>.ca-cluster`. Granting tenants Gardener project membership would break that assumption.
====

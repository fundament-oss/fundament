...
The organization is the top-level tenant boundary. It represents a company or team. Organizations have two name fields: an immutable `name` that serves as a machine-readable identifier (DNS-compatible, used in URLs and API references), and a mutable `alias` for human consumption.
...
Organizations are not self-service. Signing in for the first time creates the user, and nothing else: the user exists without any organization until a Fundament operator assigns them to one through `funops` (see FUN-8), or an organization admin invites them. The operator can do so before the person has ever signed in, by registering the email address; the authn-api links that registration to the identity provider account on the first sign-in at that address, so the memberships are already there. A user without any organization can sign in to the console and the CLI, which tell them an operator has to add them, but can reach no organization-scoped resource.

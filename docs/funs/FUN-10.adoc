...
The primary accent color is `accent`. It is used for primary buttons, active navigation states, focus rings, and loading indicators. All interactive emphasis should use accent rather than introducing other accent colors.
...
The default body text size is `text-base`. Use `text-3xl font-semibold` for page titles, `text-lg` for section headings, and `text-sm` for badges and helper text. Use `font-mono` for technical identifiers such as UUIDs and tokens.
...
The primary icon set is the https://minbzk.github.io/storybook/?path=/story/components-content-icon--icon-gallery[NLDD icon gallery] from the `@nldd/design-system` library. Icon names are referenced using their kebab-case identifiers (e.g. `info-circle`, `check-mark`).

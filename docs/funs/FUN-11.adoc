...
spec:
  definitionRef:
    ...
    definitionHash: sha256:...  # optional only when the controller runs with PLUGIN_CONTROLLER_ALLOW_UNPINNED_HASH=true
...
link:/funs/fun-17[FUN-17], "Plugin identity and naming".

Status fields: `phase`, `message`, `ready`, `observedGeneration`, `namespace`.

=== Installation lifecycle

When a `PluginInstallation` is created, the controller:

. Validates that `metadata.name` equals `<organizationName>--<pluginName>` (see link:/funs/fun-17[FUN-17], "Plugin identity and naming").
...
. Creates a namespace `plugin-{metadata.name}` (or a truncated, hashed form if that would exceed 63 chars).
. Creates a ServiceAccount named `plugin` in that namespace — every namespace-scoped child is named `plugin`, not `plugin-{metadata.name}`, since the namespace already disambiguates.
. Creates a RoleBinding binding the ServiceAccount to the built-in `admin` ClusterRole within the plugin namespace.
. Materialises a `ClusterRole`/`ClusterRoleBinding` from the pinned `PluginDefinition`'s `permissions.rbac`.
. Creates a Deployment named `plugin` in the plugin namespace with the definition's container image.
...
* `plugins.fundament.io/organization: {organizationName}` (when it fits a label value)
* `plugins.fundament.io/plugin: {pluginName}` (when it fits a label value)
...
NOTE: The public marketplace — where external organizations publish plugins and Fundament reviews them — is specified in link:/funs/fun-20[FUN-20]. It is built on the `appstore` schema described below rather than on a store of its own, extending it with review state and listing fields. The console install flow is unchanged.
...
. Organization Admin in the `system` organization installs Plugin "cert-manager" into Cluster "foo" at version `v1.17.2`.
. A `PluginInstallation` CR named `system--cert-manager` is created; `plugin-controller` reconciles it.
. `plugin-controller` creates namespace `plugin-system--cert-manager`, ServiceAccount, RoleBinding, and Deployment.

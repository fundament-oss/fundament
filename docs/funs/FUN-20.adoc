...
All three APIs are implemented. The marketplace frontend is separate work.
...
* Which store marketplace reviewers authenticate against. `tenant.users` and `dcim.users` are separate, which is why `reviewer_user_id` is namespaced apart from `submitter_user_id`. Until that is settled, `marketplace-admin-api` accepts the console `UserToken` and relies on the admin-host restriction; a reviewer relation in the OpenFGA model is part of the same follow-up.

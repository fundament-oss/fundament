...
| `plugin:ready` | _(no payload)_ | Sent on script load. Signals that the plugin is ready to receive context.
...
| `fundament:init` | `{ theme, pluginName, crdKind, view }` | Sent after `plugin:ready`. Provides all context the plugin needs to render.

...
funops user create bart@acme-corp.com
...
* **Before the first sign-in**: `funops organization member add <organization> <email>` with an address nobody has signed in with yet registers the user by email (as `funops user create <email>` does on its own) and assigns the membership to that registration. The authn-api links the registration to the identity provider account on the first sign-in at that address, matched case-insensitively.

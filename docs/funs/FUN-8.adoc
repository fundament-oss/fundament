...
Users are addressed by ID or by email, never by name: names are what the identity provider reports and are not unique. `funops organization member remove` revokes a membership the way the organization-api does, and the authz outbox takes the change to OpenFGA.

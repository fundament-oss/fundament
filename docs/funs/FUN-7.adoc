...
In-cluster authorization is intentionally simple for now. The RoleBindings grant either read or write access to standard Kubernetes resources (deployments, services, pods, etc.) based on the user's project permission: project admins are bound to the built-in `admin` ClusterRole and project viewers to the built-in `view` ClusterRole. Organization admins get no RoleBindings; their ServiceAccount is bound to `cluster-admin` instead.

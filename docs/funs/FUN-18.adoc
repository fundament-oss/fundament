...
* `nldd.css` — NLDS global styles and tokens *plus the host's token overrides*, with the woff2 fonts **inlined as base64 `data:` URIs**.
...
Serving NLDS's stock `global.css` is not enough: the Console overrides some NLDS tokens on `:root`, and a plugin that misses them renders subtly unlike the page it is embedded in. NLDS ships an 18px base font, for example, while the Console pins it to 16px — an unpatched iframe visibly renders larger than the Console around it.

`nldd.css` is therefore built from `console-frontend/src/plugin-sdk/nldd.css`, which imports NLDS and re-applies those overrides after it. Any `:root` token the Console overrides in `src/styles.css` and that plugins should inherit belongs there too, or the two drift. (The values are duplicated rather than shared, because `styles.css` reaches them through Tailwind's `theme()` and the plugin bundle is plain CSS.)

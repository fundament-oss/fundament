...
`plugin-proxy` forwards `/installations/{id}/runtime/*` and `/installations/{id}/controller/*` requests to the plugin pod and the plugin-controller, not to the Kubernetes API. There is no kube scope to apply, so authorization reduces to: `aud=fundament-plugin`, the URL's `installation_id` matches the claim, and OpenFGA `can_view` on `(user, cluster)` still holds. The OpenFGA result is cached per `(user, cluster)` for a few seconds to keep the hot path fast; revocation lag is bounded by that cache plus the 15-minute token TTL.

`kube-api-proxy` enforces the same OpenFGA gate plus two more. The plugin-scope half: the proxy parses the K8s URL into `(apiGroup, resource, verb, resourceName?)` and matches it against the current scope on the `PluginInstallation` CR (served from a local informer cache, see below). A cert-manager plugin token that lists pods is rejected with 403 before the request reaches the cluster, regardless of who the user is. The user-permission half is enforced via the existing per-user ServiceAccount token that `kube-api-proxy` injects downstream. That SA token's permissions inside the cluster are exactly the user's project memberships, as today. A request that satisfies the plugin's scope but exceeds the user's project permissions is rejected by the cluster's own RBAC. Namespace-level enforcement lives here: a plugin scope says "read Certificates"; the user's SA decides *in which namespaces*.

The mechanism is deliberately defense-in-depth. Any gate failing produces a denial; all succeeding produces an allow.
...
- The user loses cluster access (removed from org or project) → OpenFGA `can_view` returns false at mint time and at every subsequent proxy request → 403 once the per-proxy OpenFGA cache (a few seconds) drains. In-flight streams die when the user's SA loses its cluster RoleBindings.

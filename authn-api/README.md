...
| `OIDC_REDIRECT_URL` | No | `http://authn.fundament.localhost:10080/callback` | Callback URL registered with the OIDC provider. Must match exactly. |
| `FRONTEND_URL` | No | `http://console.fundament.localhost:10080` | Default URL to redirect users after successful login. |
| `DATABASE_URL` | No | `postgres://authn_api:password@localhost:5432/fundament` | PostgreSQL connection string. |
| `LISTEN_ADDR` | No | `:10080` | Address and port for the HTTP server to listen on. |
| `LOG_LEVEL` | No | `info` | Logging level: `debug`, `info`, `warn`, or `error`. |
| `CORS_ALLOWED_ORIGINS` | No | `http://console.fundament.localhost:10080` | Comma-separated list of allowed CORS origins. |

...
# Or run directly
cd authn-api && JWT_SECRET=abc LISTEN_ADDR=:10100 go run ./cmd/fun-authn-api
...
- **Gorilla Sessions** for OAuth state management (CSRF protection + PKCE)
...
- `OIDC_REDIRECT_URL` (http://authn.fundament.localhost:10080/callback)
- `FRONTEND_URL` (http://console.fundament.localhost:10080)
- `DATABASE_URL` (postgres://authn_api:password@localhost:5432/fundament)
- `LISTEN_ADDR` (:10080)
- `LOG_LEVEL` (info)
- `CORS_ALLOWED_ORIGINS` (http://localhost:5173,http://localhost:4200,http://console.fundament.localhost:10080)

...
Bart is a backend engineer at Acme Corp. He uses Fundament primarily to understand the state of the clusters his services run on — checking which plugins are active, reviewing cluster health, and looking up configuration details when debugging production issues. Bart cannot make changes himself; when he identifies something that needs updating, he raises it with Alice.

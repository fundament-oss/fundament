...
function renderForm(o: { name?: string; kind?: string; target?: string; requests?: string; unit?: string }): HTMLFormElement {
...
describe('buildBackendTrafficPolicyBody', () => {
  it('omits rateLimit when no request count', () => {
    const body = buildBackendTrafficPolicyBody(renderForm({ target: 'demo' }), 'team-a');
  ...
  it('builds a local rate limit with the chosen unit', () => {
    const body = buildBackendTrafficPolicyBody(renderForm({ requests: '100', unit: 'Minute' }), 'team-a');
  ...
  it('defaults the unit to Second', () => {
    const body = buildBackendTrafficPolicyBody(renderForm({ requests: '5' }), 'team-a');

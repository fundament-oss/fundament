...
try {
  ...
  content.textContent = `Failed to load the plugin SDK: ${err instanceof Error ? err.message : err}`;
  ctx = null;
}
...
if (ctx && !ctx.resource?.name) {
  ...
  try {
    const item = await window.fundament.k8s.get<FSCInstallation>({
    ...
    // Only link http(s) URLs: anything else (javascript:, data:) would execute
    // when clicked.
    ...
    content.textContent = `Failed to load: ${err instanceof Error ? err.message : err}`;
  }
}

...
func (c *perShootCache[T]) invalidate(clusterID uuid.UUID) {
	...
	delete(c.lastRetry, clusterID)
	c.mu.Unlock()
}

// pruneLocked drops expired entries and retry marks older than negativeTTL,
// so clusters that are deleted or no longer queried do not stay in memory.
// It runs on every write; both maps hold at most one item per cluster.
func (c *perShootCache[T]) pruneLocked() {
	now := c.now()
	...
	for id, last := range c.lastRetry {
		if now.Sub(last) >= negativeTTL {
			delete(c.lastRetry, id)
		}
	}
}

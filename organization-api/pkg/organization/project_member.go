...
func (s *OrganizationServer) AddProjectMember(
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) {
			if pgErr.Code == "23505" && pgErr.ConstraintName == dbconst.ConstraintProjectMembersUqProjectUser {
...
func (s *OrganizationServer) UpdateProjectMemberRole(
	...
	if err != nil {
		var pgErr *pgconn.PgError
		if errors.As(err, &pgErr) {
			if pgErr.Code == "P0001" && pgErr.Hint == dbconst.HintProjectContainsOneAdmin {
...
// isLastAdminError checks if the error is from the database trigger that prevents
// removing or demoting the last admin of a project.
// TODO: This is fragile because it matches on the error message text from the database trigger.
// If the message in project_members_tr_protect_last_admin() is changed, this check will break silently.
// Consider using a custom SQLSTATE error code (e.g., RAISE EXCEPTION ... USING ERRCODE = 'FUN01')
...
func isLastAdminError(err error) bool {
	...
	var pgErr *pgconn.PgError
	if errors.As(err, &pgErr) {
		return pgErr.Code == "P0001" && pgErr.Hint == dbconst.HintProjectContainsOneAdmin
	}
	return false
}

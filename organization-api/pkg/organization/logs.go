...
func callerAuthHeaders(h http.Header) http.Header {
	out := http.Header{}
...
func (s *Server) QueryLogs(
	...
	client = s.pluginPodClient(ctx, client, &params, callerAuthHeaders(req.Header()))
...
func (s *Server) GetLogLabels(
	...
	labels, err := client.Labels(ctx, clusterID.String(), req.Msg.GetNamespace(), start, end)
	if err != nil {
		if isDegradableLogError(err) {
			s.logger.WarnContext(ctx, "logs backend unreachable, degrading", "cluster_id", clusterID, "error", err)
			return connect.NewResponse(organizationv1.GetLogLabelsResponse_builder{

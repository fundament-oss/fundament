...
// versions + machine types). Global catalog data: authenticated like every RPC,
// but no permission check or org filter (same pattern as ListPresets).
func (s *Server) ListRegions(
	...
	versionsByRegion := make(map[uuid.UUID][]*organizationv1.RegionKubernetesVersion)
	for _, v := range versions {
		versionsByRegion[v.RegionID] = append(versionsByRegion[v.RegionID], organizationv1.RegionKubernetesVersion_builder{
			Id:      v.ID.String(),
			Version: v.Version,
		}.Build())
	}

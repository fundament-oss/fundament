 ...
 *
 * A sheet or modal dialog is a <dialog> opened with showModal(), in the
 * browser's top layer and painting over everything outside it. The design
 * system carries the region into the topmost open overlay itself, so a message
 * raised from inside a sheet stands in front of it.
 */
@Injectable({
  providedIn: 'root',
})

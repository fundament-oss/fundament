...
export default class PluginIframeComponent implements OnInit {
  ...
  private pluginProxyOrigin = computed(() => {
    // Prefer the runtime config; fall back to window.location.origin for the
    // bundled demo plugin (whose src is same-origin `/plugin-ui/...`).
    return this.configService.getConfig().pluginProxyUrl || window.location.origin;
  });

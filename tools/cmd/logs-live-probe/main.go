...
//	PROMETHEUS_CA_FILE=<PEM bundle, when the seed ingress CA is private> \
...
func main() {
	...
	// Same CA knob as production; no insecure fallback.
	var opts []logs.Option
	transport := http.DefaultTransport
	caFile := os.Getenv("PROMETHEUS_CA_FILE")
	if caFile != "" {
		t, err := prom.TransportWithCA(caFile)
		fatal("PROMETHEUS_CA_FILE", err)
		transport = t
		opts = append(opts, logs.WithTransport(t))
	} else if !defaultOK {
		fatal("tls", fmt.Errorf("default transport cannot verify %s and PROMETHEUS_CA_FILE is not set", info.URL))
	}

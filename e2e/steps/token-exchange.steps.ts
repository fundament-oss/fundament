...
Given(
  'I have deleted the API key for exchange test',
...
When(
  'I call ExchangeToken with the API token',
  async function (this: ICustomWorld) {
    try {
...
When(
  'I call ExchangeToken without an Authorization header',
  async function (this: ICustomWorld) {
    // Call without any token
    try {
      const response = await fetch(
        `${this.authnApiUrl}/authn.v1.TokenService/ExchangeToken`,
        {
          method: 'POST',
          headers: {
...
When(
  'I call ExchangeToken with token {string}',
...
When(
  'I call ExchangeToken with a valid-format but non-existent token',
  async function (this: ICustomWorld) {
...
Then(
  'the JWT should expire in {int} seconds',
...
Then(
  ...
  async function (this: ICustomWorld, tokenType: string) {
    const response = this.lastApiResponse as TokenLikeResponse | undefined;
    expect(response).toBeDefined();
...
Then(
  'the last used timestamp should be recent',
  async function (this: ICustomWorld) {

...
func New(kubeconfigPath string, logger *slog.Logger) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("build gardener controller-runtime client: %w", err)
	}
...
func (c *Client) FindShoot(ctx context.Context, clusterID string) (*gardencorev1beta1.Shoot, error) {
	...
	if len(shootList.Items) > 1 {
		// Admin-credentialed traffic must never route to an arbitrary shoot.
		return nil, fmt.Errorf("found %d shoots labeled with cluster %s, expected exactly one", len(shootList.Items), clusterID)
	}

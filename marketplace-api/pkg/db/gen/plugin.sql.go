...
func (q *Queries) PluginLabelsList(ctx context.Context) ([]AppstorePluginLabel, error) {
	rows, err := q.db.Query(ctx, pluginLabelsList)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AppstorePluginLabel
	for rows.Next() {
		var i AppstorePluginLabel
...
func (q *Queries) PluginLabelsListByPluginID(ctx context.Context, arg PluginLabelsListByPluginIDParams) ([]AppstorePluginLabel, error) {
...
WHERE
  ...
  AND ($3::bool = false OR appstore.plugins.featured)
ORDER BY
  CASE WHEN $4::text = 'featured' AND appstore.plugins.featured THEN 0 ELSE 1 END,
  CASE WHEN $4::text = 'recently_added' THEN (
    SELECT MIN(appstore.plugin_definitions.published)
    FROM appstore.plugin_definitions
    WHERE appstore.plugin_definitions.plugin_id = appstore.plugins.id
  ) END DESC NULLS LAST,

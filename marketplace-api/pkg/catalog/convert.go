...
// Sort discriminators passed to PluginList. UNSPECIFIED sorts as NAME, which is
// also PluginList's tiebreaker, so an unset sort needs no special casing there.
const (
...
func sortKey(sort catalogv1.PluginSort) string {
	switch sort {
	case catalogv1.PluginSort_PLUGIN_SORT_UNSPECIFIED, catalogv1.PluginSort_PLUGIN_SORT_NAME:
		return sortName

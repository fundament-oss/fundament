...
func (s *Server) replaceDocumentationLinks(ctx context.Context, qtx *db.Queries, pluginID uuid.UUID, links []*marketplacev1.DocumentationLink) error {
	...
	if err := qtx.RegistryPluginDocLinksSoftDeleteNotIn(ctx, db.RegistryPluginDocLinksSoftDeleteNotInParams{
		PluginID: pluginID,
		KeptIds:  kept,
	}); err != nil {
	...
	for position, link := range links {
		...
		updated, err := qtx.RegistryPluginDocLinkUpdate(ctx, db.RegistryPluginDocLinkUpdateParams{
...
func (s *Server) replaceFeatures(ctx context.Context, qtx *db.Queries, pluginID uuid.UUID, features []*marketplacev1.FeatureBlock) error {
	...
	if err := qtx.RegistryPluginFeaturesSoftDeleteNotIn(ctx, db.RegistryPluginFeaturesSoftDeleteNotInParams{
		PluginID: pluginID,
		KeptIds:  kept,
	}); err != nil {
	...
	for position, feature := range features {
		...
		updated, err := qtx.RegistryPluginFeatureUpdate(ctx, db.RegistryPluginFeatureUpdateParams{

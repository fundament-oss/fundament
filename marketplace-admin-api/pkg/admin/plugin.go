...
func (s *Server) GetPluginVersion(
	...
	if err != nil {
		...
		// data problem, not a request problem. The version still renders, but
		// the failure travels on the response: empty capabilities must read as
		// "could not be derived", never as "asks for nothing" — a reviewer
		// approving on the latter would publish RBAC that was never displayed.

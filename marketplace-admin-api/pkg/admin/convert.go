...
// statusFilterToDB maps the ListSubmissions filter. UNSPECIFIED becomes the
// empty string, which the query reads as "every status".
func statusFilterToDB(status marketplacev1.SubmissionStatus) string {
	switch status {
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_UNSPECIFIED:
		return ""
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_DRAFT:
		return string(dbconst.PluginDefinitionStatus_Draft)
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_PENDING:
		return string(dbconst.PluginDefinitionStatus_Pending)
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_CHANGES_REQUESTED:
		return string(dbconst.PluginDefinitionStatus_ChangesRequested)
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_APPROVED:
		return string(dbconst.PluginDefinitionStatus_Approved)
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_REJECTED:
		return string(dbconst.PluginDefinitionStatus_Rejected)
	case marketplacev1.SubmissionStatus_SUBMISSION_STATUS_WITHDRAWN:
		return string(dbconst.PluginDefinitionStatus_Withdrawn)
	default:
		panic("unhandled SubmissionStatus: " + status.String())
	}
}
